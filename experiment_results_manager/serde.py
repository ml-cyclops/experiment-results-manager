...
def write_to_file(str_or_bytes: str | bytes, uri: str) -> None:
...
def save_experiment_run_to_registry(
    er: ExperimentRun, experiment_registry_path: str
) -> str:
    run_path = posixpath.join(
        experiment_registry_path, er.experiment_id, er.variant_id, er.run_id
    )
    save_experiment_run_to_path(er, run_path)
    return run_path


def save_experiment_run_to_path(er: ExperimentRun, path: str) -> None:
...
def load_experiment_run_from_registry(
    ...
    return load_experiment_run_from_path(run_path)


def load_experiment_run_from_path(run_path: str) -> ExperimentRun:
